...
- **WEA-15**: In Progress — code fix committed, tarballs deployed to `hetzner-dev-box-config/herdctl/tarballs/`, awaiting devops rebuild and test
- **Tarballs updated**: Both `herdctl-core-3.0.2.tgz` and `herdctl-slack-0.1.0.tgz` in `~/hetzner-dev-box-config/herdctl/tarballs/`
