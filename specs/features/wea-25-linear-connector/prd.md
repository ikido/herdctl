...
worktree:
  enabled: true
  base_dir: /workspace/issues  # Creates: issues/eng-123-fix-auth-bug
  setup_script: .herdctl/setup-worktree.sh  # Optional: project-specific setup
  cleanup_script: .herdctl/cleanup-worktree.sh  # Optional: project-specific cleanup
  handoff_dir: specs  # Handoff docs go in specs/ subfolder
```

**Setup Script** (`.herdctl/setup-worktree.sh`) - **Optional, project-specific**:
- Runs after worktree creation if file exists
- Can generate `.env` files, install dependencies, etc.
- Available env vars: `LINEAR_ISSUE_ID`, `LINEAR_ISSUE_IDENTIFIER`, `LINEAR_ISSUE_TITLE`, `LINEAR_WORKTREE_PATH`, `LINEAR_BRANCH_NAME`, `LINEAR_HANDOFF_DIR`

**Cleanup Script** (`.herdctl/cleanup-worktree.sh`) - **Optional, project-specific**:
- Runs before worktree deletion if file exists
- Can backup data, clean up resources, etc.
- Same env vars available as setup script
...
- **Own branch**: `issues/eng-123-fix-auth-bug` (includes `issues/` prefix)
...
- **Other agent comments** trigger this agent **only if this agent is @mentioned**
...
Example:
```
Comment: "@backend-agent implement OAuth, @security-agent review"
→ backend-agent triggered (mentioned)
→ security-agent triggered (mentioned)

Comment: "backend-agent completed the work"
→ No agent triggered (no @mention)
```
...
/workspace/issues/eng-123-fix-auth-bug/  # Worktree directory
  └── specs/                              # Handoff directory (required)
      └── eng-123-fix-auth-bug/           # Issue-specific docs
          ├── 2026-02-17-initial-plan.md  # Timestamped handoff docs
          └── 2026-02-18-progress.md      # Generated by /handoff command
```

**Note**: No assumptions about project structure. Only `specs/` handoff directory is created. Project-specific setup (`.env`, `src/`, etc.) is handled by optional setup script.

**Branch naming**: `issues/eng-123-fix-auth-bug` (includes `issues/` prefix)
...
- `LINEAR_BRANCH_NAME`: `issues/eng-123-fix-auth-bug`
- `LINEAR_HANDOFF_DIR`: `/workspace/issues/eng-123-fix-auth-bug/specs/eng-123-fix-auth-bug`

**Setup script** (`.herdctl/setup-worktree.sh`) - **Optional**:
```bash
#!/bin/bash
# Example: project-specific setup
# Only runs if this file exists

# Generate .env file (optional, project-specific)
cat > "$LINEAR_WORKTREE_PATH/.env" <<EOF
DATABASE_URL=postgres://localhost/test
API_KEY=test-key
EOF

# Install dependencies (optional, project-specific)
cd "$LINEAR_WORKTREE_PATH"
npm install
```

**Cleanup script** (`.herdctl/cleanup-worktree.sh`) - **Optional**:
```bash
#!/bin/bash
# Example: project-specific cleanup
# Only runs if this file exists

# Backup any important data before worktree deletion
...
issues:
  <issue-uuid>:
    ...
    branchName: "issues/eng-123-fix-auth-bug"
...
3. Git worktree add:
   ...
   - Branch: issues/eng-123-fix-auth-bug
   - Base: main (or parent issue branch for sub-tasks)
4. Create handoff directory:
   - mkdir -p specs/eng-123-fix-auth-bug/
5. Run setup script (.herdctl/setup-worktree.sh) if exists:
   - Project-specific initialization
   - Optional: generate .env, install deps, etc.
6. Mount worktree into agent container
7. Spawn agent with LINEAR_* env vars
```

### Branch Naming Convention

Branch name includes `issues/` prefix:

```
issues/<issue-identifier>-<sanitized-title>

Examples:
issues/eng-123-fix-auth-bug
issues/eng-456-add-dark-mode
```

**Rationale**: Namespaced branches, easy to filter (`git branch --list 'issues/*'`)

### Sub-Issue Branching

If issue has parent:
- Base branch = parent's issue branch (not main)
- Example: Parent `issues/eng-100-oauth` → Sub-issue `issues/eng-123-add-pkce` branches from `issues/eng-100-oauth`
- Allows stacked PRs
...
interface LinearSession {
  ...
  branchName?: string;            // "issues/eng-123-fix-auth-bug"
...
Agent should use `/handoff` command **if available** to create timestamped context documents in `specs/<issue-identifier>/`:
- `2026-02-17-initial-plan.md` - Initial implementation plan
- `2026-02-17-research-findings.md` - Research and findings
- `2026-02-18-progress-update.md` - Session progress/handoff

**Timestamped format**: `YYYY-MM-DD-<slug>.md`

**If /handoff command not available**:
- Agent manually creates markdown files in specs/ directory
- Same timestamped naming convention

These files help with:
- Session continuity after `!reset`
- Context handoff (WEA-22)
- Human review and understanding
- Multi-agent collaboration (other agents can read handoff docs)
