...
chat:
  discord:
    ...
    output:
      ...
      result_summary: false       # Show completion summary embed (default: false)
...
| `output.result_summary` | boolean | `false` | Show a summary embed when the agent finishes (duration, cost, tokens) |
...
<Aside type="tip">
  Set `result_summary: true` if you want to see how long each response took and how many tokens were used. This is useful during development but can be noisy in production.
</Aside>
