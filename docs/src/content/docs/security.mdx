...
<Aside type="tip">
  Docker options are split into two tiers: **safe options** (allowed in agent config) and **dangerous options** (fleet config only).
</Aside>
...
<Aside type="note">
  The `env` option is restricted to fleet config because environment variables could be used to inject credentials or modify agent behavior. This is part of the [tiered security model](#tiered-configuration-security).
</Aside>
