...
3. **Phase 2: Run security audit (INLINE)**
   - Include the 5-phase audit process INLINE as <step> sections (not subagent spawns)
   - This is the key transformation: Task tool spawns become inline <step> sections
   - Conditional logic still applies ("IF commits > 0") but executes inline steps instead of spawning
   - The subagent prompts (from security-audit.md) become inline instructions

   **Transformation pattern from /security-audit.md:**
   - "Spawn change-analyzer agent" → Include <step name="inline_change_analysis"> with the agent's instructions
   - "IF VERIFY recommended: spawn hot-spot-verifier" → Include <step name="inline_hot_spot_verification"> with conditional execution
   - "IF High priority questions: spawn question-investigator" → Include <step name="inline_question_investigation"> with conditional execution

   **Key phases to include inline:**
   - Scanner phase (run pnpm security --json)
   - Change detection (git log since last audit, categorize changes)
   - Hot-spot verification (conditional - if VERIFY recommended)
   - Question investigation (conditional - if High priority questions exist)
   - Aggregation (determine PASS/WARN/FAIL)
...
**Scope note:** This is a 300-500 line orchestration command, not complex code. Focus on clear structure over verbosity - the <step> tags provide natural organization. Use reference comments like "# See /security-audit.md for full scanner details" to avoid duplication where appropriate.

Keep the command under 500 lines. Focus on the orchestration structure; the inline audit process uses <step> sections with the key instructions embedded.
  </action>
  <verify>
```bash
...
**This is a verification checkpoint with explicit pass/fail:**
- **PASS:** All 7 agents present in .claude/agents/security/ → INT-03 satisfied
- **FAIL:** One or more agents missing → Note which are missing in summary; this indicates a gap from prior phases that needs investigation

Note: Prior phases (02-06) already created these agents in the correct location. This task validates that work was completed correctly. No remediation is needed here because agents were created in place - if any are missing, it means a prior phase did not complete as expected.
  </action>
  <verify>
```bash
# Count agents
AGENT_COUNT=$(ls .claude/agents/security/*.md 2>/dev/null | wc -l | tr -d ' ')
echo "Agent count: $AGENT_COUNT"

# List all agents
ls .claude/agents/security/

# Verify count is exactly 7
[ "$AGENT_COUNT" -eq 7 ] && echo "PASS: All 7 agents present" || echo "FAIL: Expected 7, found $AGENT_COUNT"
```
  </verify>
  <done>
**PASS condition:** All 7 security agents confirmed in `.claude/agents/security/` directory, satisfying INT-03.

**FAIL condition:** Missing agents identified and documented. Note: This should not occur if prior phases completed correctly - investigate Phase 02-06 summaries if FAIL.
  </done>
</task>
